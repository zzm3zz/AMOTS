...
class DataLoader3D_partial(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, different_partial_type_keys=None, is_partial=False):
        """
        This is the basic data loader for 3D networks. It uses preprocessed data as produced by my (Fabian) preprocessing.
        You can load the data with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the data loader will unpack them on the fly. This may take a while
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3D_partial, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
 ...
 #       print(different_type_cnt, index)
    ...
    def generate_train_batch(self):
        ...
        if self.is_partial:
            ...
            self.indices = list(set(self.list_of_keys).intersection(set(selected_cases)))
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'partial_type':[selected_partial_type]*self.batch_size}
    
    
class DataLoader3D_mots(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, different_partial_type_keys=None, is_partial=False):
        """
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3D_mots, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.different_partial_type_keys={k:v for k,v in 
            different_partial_type_keys.items() if len(v) >= batch_size}
    ...
    def generate_train_batch(self):
        ...
        if self.is_partial:
            ...
            selected_cases = self.different_partial_type_keys[selected_partial_type]
            # self.indices = list(set(self.list_of_keys).intersection(set(selected_cases)))
            self.indices = list(set(self.list_of_keys))
            while len(self.indices) < self.batch_size:
                selected_partial_type = self._random_select_partial_type()
                selected_cases = self.different_partial_type_keys[selected_partial_type]
                # self.indices = list(set(self.list_of_keys).intersection(set(selected_cases)))
