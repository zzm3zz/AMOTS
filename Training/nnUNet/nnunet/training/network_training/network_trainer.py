...
#                 if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, "model_best.model"))
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA and self.epoch > 500:
                self.best_val_eval_criterion_MA = self.val_eval_criterion_MA
                #self.print_to_log_file("saving best epoch checkpoint...")
                st = "model_best.model" + str(self.epoch)
                if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, st))
